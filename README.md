...
마인크래프트 Paper 1.21.11 서버를 위한 16개의 고유한 직업 시스템을 갖춘 PVP 플러그인입니다.

플러그인 다운: https://hangar.papermc.io/Dangel165/ChaosPVP

## 주요 기능

- **16개의 고유 직업**: 검사, 과학자, 역병 살포자, 방패 병사, 크리티컬 커터, 항해사, 선장, 생명체, 저글러, 용의 분노자, 언데드, 스탬퍼, 시간 각인자, 기마병, 활력 절단자, 마라토너
...
### 기마병 (Cavalry)
말을 타고 전장을 누비는 기동성 직업입니다.
- **패시브**: 영구 둔화 I 효과
- **승마&하마**: 말 소환 (플레이어와 체력 공유), 하마 시 체력 6 회복 (3하트), 22초 쿨타임
- **휩쓸기**: 전방 3블록 범위 공격 - 6 피해 (기마 시 8 피해), 6초 쿨타임 (적중 시 1초씩 감소, 최소 1초)
- **특수**: 말 피해 시 플레이어도 절반 피해 받음, 말 사망 시 자동 제거

### 마라토너 (Marathoner)
달리기로 속도를 올리는 지구력 직업입니다.
- **패시브**: 5초 달리기 → 신속 I, 3.5초 더 → 신속 II, 3.5초 더 → 신속 III, 3.5초 더 → 신속 IV + 힘 II
- **패시브 해제**: 달리기 중단, 피해, 점프 시 모든 버프 제거
- **크라우칭 스타트**: 신속 III (0.5초), 5초 쿨타임

### 활력 절단자 (Vitality Cutter)
적의 생명력을 흡수하는 흡혈 직업입니다.
- **패시브**: 시작 시 돌검 없음
- **활력 절단**: 좌클릭 공격 시 대상 현재 체력의 1/6만큼 추가 즉시 피해
- **생명력 흡수**: 공격 시 1 HP (0.5하트) 회복
- **특수**: 팀전에서 팀원 공격 시 패시브 미적용

## 설치 방법

1. https://hangar.papermc.io/Dangel165/ChaosPVP 에서 플러그인 다운로드
...
### v3.0.3 BETA (2026-03-01)

#### 기능 추가
- **게임 시작 음악 시스템 (선택 사항)**: 게임 시작 시 음악 재생 기능 추가
  - 3가지 음악 모드 지원:
    1. 마인크래프트 내장 사운드 (예: MUSIC_DISC_PIGSTEP)
    2. 유튜브 링크 (채팅에 클릭 가능한 링크 전송, 브라우저 열림)
    3. 커스텀 음악 (리소스팩 사용)
  - 음악은 기본적으로 비활성화되어 있으며, 설정 시에만 재생
  - 맵 도착 10초 후 자동 재생 (프리즈 기간 이후)
  - 게임 종료 시 자동 정지
  - `/음악설정` 명령어로 설정 가능
- **게임 중 접속 시 관전 모드 강제**: 게임 진행 중 접속한 플레이어는 클래스 보유 여부와 관계없이 자동으로 관전 모드로 전환
  - 게임 중 OP 권한 자동 제거
  - 게임 종료 시 크리에이티브 모드 및 OP 권한 복구
- **무한 야간투시 효과**: 모든 플레이어에게 접속 시 무한 야간투시 효과 자동 부여
  - 입자 효과 및 아이콘 표시 없음
  - 게임 플레이 시인성 향상
- **클래스 아이템 드롭 방지 시스템**: 직업 전용 아이템 실수 드롭 방지
  - 일반 아이템: 자유롭게 드롭 가능
  - 클래스 아이템: 드롭 시도 시 "§c클래스 아이템은 버릴 수 없습니다!" 메시지 표시
  - 예외: 과학자는 팀전에서 클래스 아이템 드롭 가능 (팀원 지원 목적)

#### 버그 수정
- **클래스 밴 투표 동점 처리 개선**: 최다 득표 클래스가 여러 개일 때 모든 동점 클래스 중에서 동일한 확률로 랜덤 선택
  - 기존: 마지막 2개 클래스만 비교
  - 개선: 모든 동점 클래스를 리스트에 수집 후 랜덤 선택

### v3.0.3 

#### 새로운 직업 추가
- **기마병 (Cavalry)**: 말을 타고 전장을 누비는 기동성 직업
  - 패시브: 영구 둔화 I 효과
  - 승마&하마: 말 소환 및 하마 시 체력 회복
  - 휩쓸기: 전방 범위 공격 (기마 시 피해 증가)
- **마라토너 (Marathoner)**: 달리기로 속도를 올리는 지구력 직업
  - 패시브: 달리기 시간에 따라 신속 I~IV + 힘 II 획득
  - 크라우칭 스타트: 순간 가속 능력
- **활력 절단자 (Vitality Cutter)**: 적의 생명력을 흡수하는 흡혈 직업
  - 패시브: 공격 시 대상 현재 체력의 1/6만큼 추가 피해
  - 생명력 흡수: 공격 시 1 HP 회복

#### 버그 수정
- **마라토너 상태 초기화 수정**: 게임 종료 후 마라토너의 스프린트 추적 상태가 초기화되지 않아 두 번째 판에서 작동하지 않던 문제 수정
  - `GameManager.cleanupClassEffects()`에 마라토너 핸들러 정리 로직 추가
  - 게임 종료 시 모든 스프린트 추적, 속도 레벨, 모니터링 태스크 초기화
- **기마병 프리즈 기간 버그 수정**: 게임 시작 시 기마병이 움직일 수 없던 문제 수정
  - 프리즈 기간 체크를 Slowness 효과 기반에서 `GameManager.isInFreezePeriod()` 메서드 기반으로 변경
  - 기마병의 패시브 Slowness I 효과가 프리즈 기간 체크와 충돌하지 않도록 수정
  - `FreezeProtectionHandler`에서 프리즈 기간을 별도로 추적하도록 개선
 
### v3.0.2 

#### 버그 수정
- **검술가 패시브 수정**: 돌검 공격 시 +1 추가 피해가 적용되지 않던 문제 수정
  - `ignoreCancelled = false`로 변경하여 취소된 이벤트에서도 작동하도록 수정
  - 이벤트 취소 여부를 수동으로 체크하여 정상적인 공격에만 적용
 
- **크리티컬 커터 기본 무기 피해량 수정**: 기본 무기가 6 피해가 아닌 5 피해로 적용되도록 수정
  - 철검 사용하되 데미지를 5.0으로 직접 설정 (돌검과 동일)
  - 치명타 발생 시 조정된 5.0 데미지를 기준으로 추가 즉시 피해 적용
 
- **기본 게임 모드 변경**: 기본 게임 모드를 팀전에서 개인전으로 변경
  - 개인전 시작 시 팀 선택 메시지가 나오던 문제 해결
  - 팀전 드래프트 픽 시스템 정상 작동
  - 
- **연습모드 사망 처리 수정**: 연습모드에서 사망 시 자동으로 연습모드가 종료되도록 수정

- **하늘섬 맵 이펙트 시스템 수정**: 
  - 시간을 13000 틱(노을/황혼)으로 변경
  - 야간투시 효과 입자 제거 (효과는 유지, 입자만 숨김)

#### 기능 추가
- **하늘섬 맵 특수 효과 시스템**:
  - 노을/황혼 시간(13000 틱)으로 고정
  - 맵 이름에 "하늘섬", "sky", "skyisland" 포함 시 자동 감지 (대소문자 구분 없음)

- **맵 슬롯 단일 스폰 설정**:
  - `/1번맵지정` 및 `/2번맵지정` 실행 시 기존 스폰 포인트 자동 삭제
  - 현재 위치를 맵 중심 및 첫 번째 스폰 포인트로 자동 설정
  - 추가 스폰 포인트는 `/1번스폰추가` 또는 `/2번스폰추가`로 추가 가능
  - 기존 스폰 개수 알림 메시지 추가
